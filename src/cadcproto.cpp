...
bool cADCProto::ValidateRouting(cMessageADC *msg, nSocket::cAsyncConn *conn)
{
	...
	// I is generated by the hub, U is UDP and C is a client-client TCP
	// context. None is valid as input on this client-hub TCP connection.
...
int cADCProto::RouteNormal(cMessageADC *msg, nSocket::cAsyncConn *conn)
{
	...
	// INF is sanitized by the server adapter because PD is private. Feature
	// broadcasts need the recipient SU set and are handled separately.
...
int cADCProto::TreatMsg(cMessageParser *parser, nSocket::cAsyncConn *conn)
{
	...
	switch (msg->mType) {
		...
		case eADC_QUI:
			// Keep the session until socket cleanup so OnDisconnect can notify
			// the remaining users with IQUI before detaching it.
			return 0;
